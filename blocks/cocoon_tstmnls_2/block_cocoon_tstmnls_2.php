...
class block_cocoon_tstmnls_2 extends block_base {

    /**
     * Start block instance.
     */
    function init() {
        $this->title = get_string('pluginname', 'block_cocoon_tstmnls_2');
    }
		...
		<div class="container">
          ...
          $fs = get_file_storage();
            for ($i = 1; $i <= $data->slidesnumber; $i++) {
                ...
                $files = $fs->get_area_files($this->context->id, 'block_cocoon_tstmnls_2', 'slides', $i, 'sortorder DESC, id ASC', false, 0, 0, 1);

                if (!empty($data->$sliderimage) && count($files) >= 1) {
                  $mainfile = reset($files);
                  $mainfile = $mainfile->get_filename();
                  $data->$sliderimage = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php","/{$this->context->id}/block_cocoon_tstmnls_2/slides/" . $i . '/' . $mainfile);
                } else {
...
<img  src="'.$data->$sliderimage.'" alt=""></div>
...
';
    ...
    function instance_config_save($data, $nolongerused = false) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            file_save_draft_area_files($data->$field, $this->context->id, 'block_cocoon_tstmnls_2', 'slides', $i, $filemanageroptions);
        }
    ...
    function instance_delete() {
        global $DB;
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'block_cocoon_tstmnls_2');
        return true;
    }
    ...
    public function instance_copy($fromid) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            if (!$fs->is_area_empty($fromcontext->id, 'block_cocoon_tstmnls_2', 'slides', $i, false)) {
                $draftitemid = 0;
                file_prepare_draft_area($draftitemid, $fromcontext->id, 'block_cocoon_tstmnls_2', 'slides', $i, $filemanageroptions);
                file_save_draft_area_files($draftitemid, $this->context->id, 'block_cocoon_tstmnls_2', 'slides', $i, $filemanageroptions);
            }
        }
