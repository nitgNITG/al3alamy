...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // الحصول على الحقول باستخدام المعرفات الخاصة بهم
        var id_price = document.getElementById('id_price');
        var id_description_editor = document.getElementById('id_description_editor');

        // Set input type to number and add min attribute to enforce minimum value
        id_price.setAttribute('type', 'number');
        id_price.setAttribute('min', '1');
        
        // أضف مستمع حدث input إلى الحقل id_price
        id_price.addEventListener('input', function() {
            // عندما يتغير محتوى id_price، يتم تحديث id_description_editor بنفس القيمة
            id_description_editor.innerText = id_price.value;
        });

        // عندما يتغير محتوى id_price، يتم تحديث id_description_editor بنفس القيمة
        id_price.value = id_description_editor.innerText;
