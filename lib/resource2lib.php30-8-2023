...
/** Try the best way */
define('resource2LIB_DISPLAY_AUTO', 0);
/** Display using object tag */
define('resource2LIB_DISPLAY_EMBED', 1);
/** Display inside frame */
define('resource2LIB_DISPLAY_FRAME', 2);
/** Display normal link in new window */
define('resource2LIB_DISPLAY_NEW', 3);
/** Force download of file instead of display */
define('resource2LIB_DISPLAY_DOWNLOAD', 4);
/** Open directly */
define('resource2LIB_DISPLAY_OPEN', 5);
/** Open in "emulated" pop-up without navigation */
define('resource2LIB_DISPLAY_POPUP', 6);

/** Legacy files not needed or new resource2 */
define('resource2LIB_LEGACYFILES_NO', 0);
/** Legacy files conversion marked as completed */
define('resource2LIB_LEGACYFILES_DONE', 1);
/** Legacy files conversion in progress*/
define('resource2LIB_LEGACYFILES_ACTIVE', 2);
 ...
 */
function resource2lib_try_file_migration($filepath, $cmid, $courseid, $component, $filearea, $itemid) {
    $fs = get_file_storage();
 ...
 */
function resource2lib_get_displayoptions(array $enabled, $current=null) {
    if (is_number($current)) {
        $enabled[] = $current;
    }

    $options = array(resource2LIB_DISPLAY_AUTO     => get_string('resource2displayauto'),
                     resource2LIB_DISPLAY_EMBED    => get_string('resource2displayembed'),
                     resource2LIB_DISPLAY_FRAME    => get_string('resource2displayframe'),
                     resource2LIB_DISPLAY_NEW      => get_string('resource2displaynew'),
                     resource2LIB_DISPLAY_DOWNLOAD => get_string('resource2displaydownload'),
                     resource2LIB_DISPLAY_OPEN     => get_string('resource2displayopen'),
                     resource2LIB_DISPLAY_POPUP    => get_string('resource2displaypopup'));
    ...
    if (empty($result)) {
        ...
        $result[resource2LIB_DISPLAY_OPEN] = $options[resource2LIB_DISPLAY_OPEN];
    }
 ...
 */
function resource2lib_guess_url_mimetype($fullurl) {
    global $CFG;
 ...
 */
function resource2lib_get_extension($fullurl) {
 ...
 * @return string html
 */
function resource2lib_embed_image($fullurl, $title) {
    $code = '';
    $code .= '<div class="resource2content resource2img">';
    $code .= "<img title=\"".s(strip_tags(format_string($title)))."\" class=\"resource2image\" src=\"$fullurl\" alt=\"\" />";
    $code .= '</div>';
 ...
 * @return string html
 */
function resource2lib_embed_pdf($fullurl, $title, $clicktoopen) {
    global $CFG, $PAGE;

    $code = <<<EOT
<div class="resource2content resource2pdf">
  <object id="resource2object" data="$fullurl" type="application/pdf" width="800" height="600">
...
EOT;
    ...
    $PAGE->requires->js_init_call('M.util.init_maximised_embed', array('resource2object'), true);
 ...
 * @return string html
 */
function resource2lib_embed_general($fullurl, $title, $clicktoopen, $mimetype) {
    global $CFG, $PAGE;
    ...
    // this is ok in HTML5.
    $code = <<<EOT
<div class="resource2content resource2general">
  <iframe id="resource2object" src="$fullurl">
    $clicktoopen
  </iframe>
</div>
EOT;
