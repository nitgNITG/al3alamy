 ...
 *
 * Confirmation uses $OUTPUT->confirm() — no JavaScript / browser dialog needed.
 *
...
if ($action === 'logout_user' && $userid) {
    ...
    if ($target && !$confirmed) {
        // Show Moodle native confirmation page — no JS needed.
        $continue_url = new moodle_url($pageurl, [
            'action'    => 'logout_user',
            'userid'    => $userid,
            'sesskey'   => sesskey(),
            'confirmed' => 1,
        ...
        $cancel_url = new moodle_url($pageurl, ['view' => 'sessions', 'filter' => $filter]);
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(
            get_string('forcelogout_confirm_all', 'local_deviceregistration')
            . '<br><strong>' . s(fullname($target)) . ' — ' . s($target->email) . '</strong>',
            $continue_url,
            $cancel_url
        );
        echo $OUTPUT->footer();
...
if ($action === 'remove_device' && $deviceid) {
    ...
    if ($device && !$confirmed) {
        ...
        $label = $device->useragent ?: get_string('unknowndevice', 'local_deviceregistration');
        ...
        $cancel_url = new moodle_url($pageurl, [
            'view'        => 'devices',
            'deviceuser'  => $device->userid,
            'filter'      => $filter,
            'countfilter' => $countfilter,
        ]);
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(
            get_string('devmgr_confirm_revoke', 'local_deviceregistration')
            . '<br><strong>' . s($label) . '</strong>'
            . ($owner ? '<br>' . s(fullname($owner)) : ''),
            $continue_url,
            $cancel_url
        );
        echo $OUTPUT->footer();
...
if ($view === 'devices') {
    ...
    if ($devrows) {
        $namefields = 'id,firstname,lastname,email,username,deleted,'
            . 'firstnamephonetic,lastnamephonetic,middlename,alternatename';
        $users = $DB->get_records_list('user', 'id', array_keys($devrows), '', $namefields);
...
.dr-badge { display:inline-block; background:#eef3f9; color:#2d6a9f; border-radius:10px;
            padding:2px 10px; font-size:.85em; font-weight:700; }
.dr-badge.warn { background:#fff3cd; color:#856404; }
.dr-empty { text-align:center; color:#888; padding:40px 20px; font-size:1.05em; }

.btn-force  { display:inline-block; background:#dc3545; color:#fff !important;
              border-radius:5px; padding:6px 16px; font-size:.86em; font-weight:600;
              text-decoration:none !important; }
.btn-force:hover  { background:#c82333; color:#fff !important; }
.btn-manage { display:inline-block; background:#2d6a9f; color:#fff !important;
              border-radius:5px; padding:6px 14px; font-size:.86em; font-weight:600;
              text-decoration:none !important; }
.btn-manage:hover { background:#245580; color:#fff !important; }
.btn-revoke { display:inline-block; background:#fff; color:#dc3545 !important;
              border:1px solid #dc3545; border-radius:5px; padding:5px 14px;
              font-size:.85em; font-weight:600; text-decoration:none !important; }
.btn-revoke:hover { background:#dc3545; color:#fff !important; }

.dr-detail { background:#f7f9fc; border:1px solid #d0dce8; border-radius:8px;
             padding:18px 20px; margin-bottom:24px; }
.dr-detail h4 { margin:0 0 12px; color:#2d6a9f; font-size:1em; }
.dr-ua { font-size:.82em; color:#555; max-width:300px; white-space:nowrap;
...
<?php
$tab_s = (new moodle_url($pageurl, ['view' => 'sessions']))->out();
$tab_d = (new moodle_url($pageurl, ['view' => 'devices']))->out();
?>
<nav class="dr-tabs">
  <a href="<?php echo $tab_s; ?>" class="dr-tab <?php echo $view==='sessions'?'active':''; ?>">
    <?php echo get_string('tab_sessions', 'local_deviceregistration'); ?>
  </a>
  <a href="<?php echo $tab_d; ?>" class="dr-tab <?php echo $view==='devices'?'active':''; ?>">
  ...
  <?php else: ?>
    ...
    <table class="dr-table">
      ...
      <?php foreach ($loggedinusers as $u):
        $logout_url = (new moodle_url($pageurl, [
            'action'  => 'logout_user',
            'userid'  => $u->id,
            'sesskey' => $sk,
            'filter'  => $filter,
      ...
      ?>
        <tr>
          ...
          <td><a href="<?php echo $logout_url; ?>" class="btn-force">
      ...
      <table class="dr-table">
        ...
        ?>
          <tr>
            ...
            <td><a href="<?php echo $revoke_url; ?>" class="btn-revoke">
