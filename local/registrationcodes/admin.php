...
if ($action && in_array($action, ['enable', 'disable', 'delete', 'deletegroup'])) {
    ...
    if ($action === 'deletegroup') {
        ...
        redirect(
            ...
            get_string('group_deleted', 'local_registrationcodes', $n),
            null,
...
if ($formdata = $generateform->get_data()) {
    ...
    $groupname = isset($formdata->groupname) ? trim($formdata->groupname) : '';
    $expiry    = !empty($formdata->timeexpiry) ? (int)$formdata->timeexpiry : null;
    $notes     = isset($formdata->notes)     ? trim($formdata->notes)     : '';

    $generated = manager::generate_codes($qty, $prefix, $expiry, $notes, $USER->id, $groupname);
...
$statdata = [
    ...
    ['label' => get_string('stats_groups',   'local_registrationcodes'), 'val' => count($groups),           'cls' => 'dark'],
];
...
if (!empty($groups)) {
    echo '<div class="card mb-4"><div class="card-body">';
    echo '<h5 class="card-title">' . get_string('groups', 'local_registrationcodes') . '</h5>';
    echo '<div class="d-flex flex-wrap gap-2">';
    $deletebaseurl = new moodle_url('/local/registrationcodes/admin.php', ['action' => 'deletegroup', 'sesskey' => sesskey()]);
    foreach ($groups as $g) {
        $filterurl = new moodle_url('/local/registrationcodes/admin.php', ['filtergroup' => $g]);
        $deleteurl = new moodle_url($deletebaseurl, ['delgroup' => $g]);
        $active    = ($filtergroup === $g) ? ' active' : '';
        echo '<div class="input-group input-group-sm" style="width:auto;">';
        echo '<a href="' . $filterurl->out(false) . '" class="btn btn-outline-primary btn-sm' . $active . '">' . s($g) . '</a>';
        echo '<a href="' . $deleteurl->out(false) . '" class="btn btn-outline-danger btn-sm"'
            . ' onclick="return confirm(\'' . addslashes(get_string('confirm_delete_group', 'local_registrationcodes', s($g))) . '\')"'
            . ' title="' . get_string('delete_group', 'local_registrationcodes') . '">✕</a>';
        echo '</div>';
    }
    echo '</div>';
    if ($filtergroup !== '') {
        echo '<div class="mt-2"><a href="' . (new moodle_url('/local/registrationcodes/admin.php'))->out(false) . '" class="btn btn-sm btn-secondary">✕ ' . get_string('clear_group_filter', 'local_registrationcodes') . '</a></div>';
    }
    echo '</div></div>';
}
...
if (empty($codes)) {
    ...
    echo '<div class="mb-2 d-flex flex-wrap gap-1">';
    ...
    foreach ($codes as $c) {
        ...
        $groupcell = '';
        if (!empty($c->groupname)) {
            $gurl      = new moodle_url('/local/registrationcodes/admin.php', ['filtergroup' => $c->groupname]);
            $groupcell = html_writer::link($gurl, s($c->groupname), ['class' => 'badge badge-info']);
        } else {
            $groupcell = '<span class="text-muted">—</span>';
        }
        ...
        if ($c->status !== manager::STATUS_USED) {
            if ($c->status === manager::STATUS_DISABLED) {
                echo html_writer::link(new moodle_url($rowurl, ['action' => 'enable']),  get_string('enable',  'local_registrationcodes'), ['class' => 'btn btn-xs btn-success mr-1']);
            } else {
                echo html_writer::link(new moodle_url($rowurl, ['action' => 'disable']), get_string('disable', 'local_registrationcodes'), ['class' => 'btn btn-xs btn-warning mr-1']);
            }
            echo html_writer::link(new moodle_url($rowurl, ['action' => 'delete']), get_string('delete', 'local_registrationcodes'), [
