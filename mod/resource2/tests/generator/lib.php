...
/**
 * resource2 module data generator class.
 *
 * @package    mod_resource2
 * @copyright 2013 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_resource2_generator extends testing_module_generator {

    /**
     * Creates new resource2 module instance. By default it contains a short
     * text file.
     *
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->files)) {
            if (empty($USER->username) || $USER->username === 'guest') {
                throw new coding_exception('resource2 generator requires a current user');
            }
            $usercontext = context_user::instance($USER->id);
            $filename = $record->defaultfilename ?? 'resource2' . ($this->instancecount + 1) . '.txt';
            ...
            $fs = get_file_storage();
            $fs->create_file_from_string($filerecord, 'Test resource2 ' . $filename . ' file');
        }
