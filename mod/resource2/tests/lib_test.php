...
/**
 * Unit tests for mod_resource2 lib
 *
...
class lib_test extends \advanced_testcase {
     ...
     * @return void
     */
    public function test_resource2_view() {
        global $CFG;
    ...
    /**
     * Tests the resource2_get_coursemodule_info function.
     *
    ...
    public function test_get_coursemodule_info() {
        ...
        // Create a resource2 with no files.
        $draftid = file_get_unused_draft_itemid();
        $resource21 = $generator->create_module('resource2', array('course' => $course->id,
                'name' => 'R1', 'files' => $draftid));

        // Create a resource2 with one file.
        $draftid = file_get_unused_draft_itemid();
        ...
        $resource22 = $generator->create_module('resource2', array('course' => $course->id,
                'name' => 'R2', 'files' => $draftid));

        // Create a resource2 with two files.
        $draftid = file_get_unused_draft_itemid();
        $filerecord = array('component' => 'user', 'filearea' => 'draft', 'contextid' => $contextid,
        ...
        $fs->create_file_from_string($filerecord, 'Test');
        $resource23 = $generator->create_module('resource2', array('course' => $course->id,
                'name' => 'R3', 'files' => $draftid));

        // Try get_coursemodule_info for first one.
        $info = resource2_get_coursemodule_info(
                $DB->get_record('course_modules', array('id' => $resource21->cmid)));
        ...
        $info = resource2_get_coursemodule_info(
                $DB->get_record('course_modules', array('id' => $resource22->cmid)));
        ...
        $info = resource2_get_coursemodule_info(
                $DB->get_record('course_modules', array('id' => $resource23->cmid)));
        $this->assertEquals('R3', $info->name);
        $this->assertEquals('f/document-24', $info->icon);
    }

    public function test_resource2_core_calendar_provide_event_action() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $event = $this->create_action_event($course->id, $resource2->id,
        ...
        $actionevent = mod_resource2_core_calendar_provide_event_action($event, $factory);
    ...
    public function test_resource2_core_calendar_provide_event_action_already_completed() {
        global $CFG;
    ...
    /**
     * Test mod_resource2_core_calendar_provide_event_action with user override
     */
    public function test_resource2_core_calendar_provide_event_action_user_override() {
        global $CFG, $USER;
        ...
        // Decorate action event.
        $actionevent = mod_resource2_core_calendar_provide_event_action($event, $factory, $USER->id);

        // Decorate action with a userid override.
        $actionevent2 = mod_resource2_core_calendar_provide_event_action($event, $factory, $user->id);
