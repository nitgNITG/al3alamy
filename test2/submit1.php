...
{
//data from vimeo AppID (Client identifier), secret token(Client secrets), token (Personal Access Tokens)
//make an object from vimeo
...
//echo $id;
//print_r($course_module->id);
// var_dump($course_module);
// if ($isadmin || $manager==1) {
$course_module_id = $DB->get_field('course_modules', 'id', array('instance' => $id, 'module' => 26));

echo "Course Module ID: " . $course_module_id;


  $url = new moodle_url("/mod/resource2/view.php", array('id' => $course_module_id));
...
';
    ...
    $type=$typeData->type;
    echo '<form name="upload-vimeo-update" enctype="multipart/form-data" id="update_form">
...
<option value="1" '; if($type==1){echo 'selected';} echo'>quiz</option>
<option value="2"'; if($type==2){echo 'selected';} echo'>Lecutre</option>
<option value="3"'; if($type==3){echo 'selected';} echo'>Homework</option>
<option value="4"'; if($type==4){echo 'selected';} echo'>Summary</option>
<option value="5"'; if($type==5){echo 'selected';} echo'>Revision</option>
</select>
  ...
  document.forms["upload-vimeo-update"].onsubmit = function(e){
    ...
    let file = this.url_update.files[0];
    let id= "' . $id . '";
    let update=1;
    ...
    formdata.append("url_update", file); 
    formdata.append("id", id); 
    formdata.append("update_form", update);
 ...
 hi
  ...
  document.forms["upload-vimeo"].onsubmit = function(e){
    ...
    http.upload.addEventListener("progress", function(event){
      let percent = (event.loaded / event.total) * 100;
      document.querySelector("progress").value = Math.round(percent);
      // document.querySelector("progress").value = event.loaded ;
      var element = document.getElementById("value");
      let uploadedSize = (event.loaded/1024/1024).toFixed(2); //change Bytes to MegaBytes
      let totalSize = (event.total/1024/1024).toFixed(2); //change Bytes to MegaBytes
      if(totalSize>2048)
      {
          alert("Files larger than 1 GB are not allowed, please contact with server administrator");
      }
      element.innerHTML = Math.round(percent) +" % " + uploadedSize.toString() + "/"+ totalSize.toString() + "Mega";
      //element.innerHTML = Math.round(percent) +" % " + (event.loaded).toString() + "/"+ (event.total).toString();      
      //element.innerHTML = + "/"+ (event.loaded).toString() ;
    });
...
console.log(this.responseText);
  ...
  if (isset($_POST['embed_upload_update'])) {
    ...
    $url = new moodle_url("/mod/resource2/view.php", array('id' => $course_module_id, 'forceview' => 1));
    redirect($url);
  ...
  //old uploading method for vimeo now we use script.php to upload and get the percent rate of uploading
...
//adding and editing videos on an external server (nit)
...
//   echo '  <script>
  ...
  //   formdata.append("resource2_idA", '.$id.'); 
  //   formdata.append("type", type); 
...
//teacher uploading to external server
...
//   if ($update == 1) {
//     $resource2 = $DB->get_record('vimeo_files2', array('resource2_id' => $id));
//     echo "<div class='container'>" . $resource2->url . "</div>";
//   }
//   $url = new moodle_url("/mod/resource2/view.php", array('id' => $course_module->id, 'forceview' => 1));
...
//   formdata.append("resource2_id", '.$id.'); 
